/**
 * Beirut wall-clock helpers for `xauusd-m1-m5-rsi-threshold-v2`.
 *
 * Deliberately self-contained, and deliberately this strategy's OWN copy
 * rather than an import from the strategy running alongside it: §1 requires
 * the two applications to share no mutable operational state and no code
 * whose removal from the other project could break this one. The technique
 * is offset-free — every offset comes from the runtime's own IANA database
 * via `Intl`, never from a hardcoded +2/+3.
 *
 ...
 * computes an offset arithmetically: whatever the host's tzdata says is what
 * the schedule uses (§9).
 */
